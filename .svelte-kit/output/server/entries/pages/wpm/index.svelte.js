import { c as create_ssr_component, f as each, e as escape, v as validate_component } from "../../../chunks/index-4ef0e261.js";
...
const WpmText = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${each(allTextArray, (word, i) => {
    return `<div class="${"word svelte-j6c4cm"}">${each(word, (charObj, j) => {
      return `${i === currentWordIndex && j === currentCharIndex && charObj.char !== " " ? `${charObj.status === "correct" ? `<span class="${"char correct blink-before svelte-j6c4cm"}">${escape(charObj.char)}</span>` : `${charObj.status === "incorrect" ? `<span class="${"char incorrect blink-before svelte-j6c4cm"}">${escape(charObj.char)}</span>` : `${charObj.status === "unfinished" ? `<span class="${"char unfinished blink-before svelte-j6c4cm"}">${escape(charObj.char)}</span>` : ``}`}`}` : `${charObj.status === "correct" ? `<span class="${"char correct svelte-j6c4cm"}">${escape(charObj.char)}</span>` : `${charObj.status === "incorrect" ? `<span class="${"char incorrect svelte-j6c4cm"}">${escape(charObj.char)}</span>` : `${charObj.status === "unfinished" ? `<span class="${"char unfinished svelte-j6c4cm"}">${escape(charObj.char)}</span>` : ``}`}`}`}`;
    })}
    </div>`;
  })}`;
});
const ElapsedTime = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<span class="${"timer"}">${escape(minutes)}:${escape(seconds)}</span>`;
});
...
const StatBar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let { netWpm } = $$props;
  let { grossWpm } = $$props;
  let { totalTime } = $$props;
  ...
  return `<div class="${"stats svelte-1qmvysy"}"><span class="${"wpm svelte-1qmvysy"}"><div class="${"net svelte-1qmvysy"}"><div class="${"title svelte-1qmvysy"}">Net WPM
    ...
    <span class="${"time svelte-1qmvysy"}"><div class="${"refresh-container svelte-1qmvysy"}"><img class="${"refresh svelte-1qmvysy"}" src="${"svg/refresh.svg"}" alt="${"refresh"}"></div>
        <div class="${"elapsed-time svelte-1qmvysy"}">${validate_component(ElapsedTime, "ElapsedTime").$$render($$result, { elapsedTime: totalTime }, {}, {})}</div></span>
</div>`;
});
...
const Results = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"result-container svelte-tqr9np"}"><div class="${"data-container svelte-tqr9np"}"><div class="${"result svelte-tqr9np"}"><div class="${"result-title svelte-tqr9np"}">Net WPM
            ...
            <span class="${"result-data svelte-tqr9np"}">${escape(netWpm)}</span></div>
        <div class="${"result svelte-tqr9np"}"><div class="${"result-title svelte-tqr9np"}">Gross WPM
            </div>
            <span class="${"result-data svelte-tqr9np"}">${escape(grossWpm)}</span></div>
        <div class="${"result svelte-tqr9np"}"><div class="${"result-title svelte-tqr9np"}">Accuracy
            </div>
            <span class="${"result-data svelte-tqr9np"}">${escape(accuracy)}%</span></div>
        <div class="${"result svelte-tqr9np"}"><div class="${"result-title svelte-tqr9np"}">Time Spent
            </div>
            <span class="${"result-data svelte-tqr9np"}">${validate_component(ElapsedTime, "ElapsedTime").$$render($$result, { elapsedTime: totalTime }, {}, {})}</span></div></div>
...
const Wpm = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `<section class="${"svelte-qs93s3"}">${!hasUserCompletedRace ? `${validate_component(StatBar, "StatBar").$$render($$result, {
    netWpm,
    grossWpm,
    totalTime,
    isRaceOngoing,
    restartTest
        ...
        <div class="${"blur svelte-qs93s3"}" tabindex="${"0"}"><div class="${"racetext svelte-qs93s3"}" id="${"racetext"}">${validate_component(WpmText, "WpmText").$$render($$result, {
    currentWordIndex,
    currentCharIndex,
    allTextArray
  }, {}, {})}</div></div>
        <div class="${"helper-text svelte-qs93s3"}"><span><i>Click the above blurred text to focus. Once focused, begin typing to start the test.</i></span></div>` : `${validate_component(Results, "Results").$$render($$result, {
    netWpm,
    grossWpm,
    accuracy,
    totalTime,
